...
export const language = pgTable(
  ...
  (language) => [
    uniqueIndex("ietf_idx").on(language.ietfCode),
    uniqueIndex("id_idx").on(language.id),
  ]
);
//@=============== WA LANG META  =============
export const waLangMetadata = pgTable(
  ...
  (table) => [
    ...
    foreignKey({
      ...
      .onDelete("cascade")
      .onUpdate("cascade"),
    uniqueIndex("wa_lang_meta_ietf_idx").on(table.ietfCode),
  ]
);
...
export const languageAlternateName = pgTable(
  ...
  (table) => [
    uniqueIndex("name_ietf_idx").on(table.name, table.languageIetfCode),
  ]
);
//@=============== LANG TO LANG PIVOT   =============
...
export const content = pgTable(
  ...
  (table) => [
    uniqueIndex("name_namespace_idx").on(table.name, table.namespace),
    index("language_idx").on(table.languageId),
  ]
);
...
export const waContentMetadata = pgTable(
  ...
  (table) => [
    uniqueIndex("content_idx").on(table.contentId),
    index("show_biel_idx").on(table.showOnBiel),
  ]
);
...
export const gitRepo = pgTable(
  ...
  (repo) => [
    uniqueIndex("username_reponame_idx").on(repo.username, repo.repoName),
    index("username_idx").on(repo.username),
    uniqueIndex("content_id_idx").on(repo.contentId),
  ]
);
...
export const repoToTopic = pgTable(
  "repo_topic",
  {
...
export const scripturalRenderingMetadata = pgTable(
  ...
  (table) => [
    ...
    foreignKey({
      ...
      .onDelete("cascade")
      .onUpdate("cascade"),
    uniqueIndex("scriptural_metadata_rendering_idx").on(table.renderingId),
  ]
);
...
export const country = pgTable(
  ...
  (table) => [uniqueIndex("alpha_2_idx").on(table.alpha2)]
);
// many-many
