...
CREATE OR REPLACE VIEW public.vw_langnames
 ...
 SELECT language.ietf_code AS lc,
    ...
    language.direction AS ld,
        CASE
            WHEN (EXISTS ( SELECT 1
              ...
              WHERE languages_to_languages.gateway_language_ietf::text = language.ietf_code::text)) THEN true
            ELSE false
        END AS gw,
    array_agg(DISTINCT country.alpha_2) AS cc,
    array_remove(array_agg(DISTINCT language_alternate_name.name), NULL::text) AS alt,
    country.alpha_2 AS hc,
   ...
   FROM language
     ...
     LEFT JOIN country ON country_to_language.country_alpha_2::text = country.alpha_2::text
     LEFT JOIN language_alternate_name ON language.ietf_code::text = language_alternate_name.ietf_code::text
     LEFT JOIN world_region ON country.world_region_id = world_region.id
  GROUP BY language.ietf_code, language.national_name, language.english_name, language.direction, country.alpha_2, world_region.region;
